...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            who = who if extra_turn(score0, score1) else other(who)
        else:
            num_rolls = strategy1(score1,score0)
            score1 += take_turn(num_rolls,score0,dice)
            who = who if extra_turn(score1,score0) else other(who)
        say(score0,score1)
    # END PROBLEM 5
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average(*args):
        ...
        while k < trials_count:
            sum, k = sum + original_function(*args), k + 1
        return sum / k
    return average
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while num <= 10:
        tmp = average(num, dice)
        if tmp > max_average:
            max_average = tmp
            max_average_num = num
        num += 1
    return max_average_num
    # END PROBLEM 9
